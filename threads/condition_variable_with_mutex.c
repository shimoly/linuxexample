...
void* thread_function (void* thread_arg)
{
	...
	while (1)
		...
		while (!thread_flag)
		/* The flag is clear. Wait for a signal on the condition variable, indicating that the flag value has changed.
		 * When the signal arrives and this thread unblocks, loop and check the flag again. */
		pthread_cond_wait (&thread_flag_cv, &thread_flag_mutex);
		/* When we’ve gotten here, we know the flag must be set. Unlock the mutex. */
		pthread_mutex_unlock (&thread_flag_mutex);
		/* Do some work. */
		do_work (*thread_id, thread_flag);
	}
	return NULL;
}
...
void set_thread_flag (int flag_value)
{
	 ...
	 *  However, thread_function can’t actually check the flag until the mutex is unlocked. */
