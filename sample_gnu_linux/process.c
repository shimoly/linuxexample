...
/* Return the name of user UID. The return value is a buffer that the caller must allocate with free. UID must be a valid user ID. */
...
/* Return the name of group GID. The return value is a buffer that the caller must allocate with free. GID must be a valid group ID. */
...
/* Return the name of the program running in process PID, or NULL on error. The return value is a newly allocated buffer which the caller must deallocate with free. */
static char *get_program_name(pid_t pid)
{
    ...
    if (fd == -1)
        /* Couldn't open the stat file for this process. Perhaps the process no longer exists. */
        return NULL;
    /* Read the contents. */
...
static int get_rss(pid_t pid)
{
    ...
    int rval;
    int rss;
    /* Generate the name of the process's "statm" entry in its /proc directory. */
...
/* Generate an HTML table row for process PID. The return value is a pointer to a buffer that the caller must deallocate with free, or NULL if an error occurs. */
static char *format_process_info(pid_t pid)
{
    ...
    /* Compute the length of the string we'll need to hold the result, and allocate memory to hold it. */
...
void module_generate(int fd)
{
    size_t i;
    DIR *proc_listing;
    /* Set up an iovec array. We'll fill this with buffers that'll be part of our output, growing it dynamically as necessary. */
    ...
    while (1)
    {
        ...
        if (proc_entry == NULL)
            ...
            break;
            /* If this entry is not composed purely of digits, it's not a process directory, so skip it. */
        ...
        if (process_info == NULL)
            /* Something went wrong. The process may have vanished while we were looking at it. Use a placeholder row instead. */
            process_info = "<tr><td colspan =\"5\"> ERROR</td></tr>";
        /* Make sure the iovec array is long enough to hold this buffer (plus one more because we'll add an extra element when we're done listing processes). If not, grow it to twice its current size. */
    ...
    /* Deallocate the buffers we created.and should not be deallocated. */
